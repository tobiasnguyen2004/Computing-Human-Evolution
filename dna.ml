...
let rec complementary_helix (x: helix) : helix =
  begin match x with
  | [] -> []
  | hd::tl -> if hd = G then C::complementary_helix tl
              else if hd = C then G::complementary_helix tl
              else if hd = A then T::complementary_helix tl
              else A::complementary_helix tl
  end
...
let rec hamming_distance (x1: helix) (x2: helix) : int =
  ...
  | [], _ -> 0
  | _ ,[] -> 0
  | hd1::tl1, hd2::tl2 -> if hd1 <> hd2 then 1 + hamming_distance tl1 tl2
                          else hamming_distance tl1 tl2
  end
...
let rec decreasing_similarity (apes: helix list) : bool =
  ...
  | hd::md::tl -> hamming_distance human hd <= hamming_distance human md 
  && decreasing_similarity (md::tl)
  | _ -> true
  end
...
let rec acid_decoder (y: helix) : acid list =
  begin match y with 
  | A::C::T::tl -> []
  | A::T::T::tl -> []
  | A::T::C::tl -> []
  | a::b::c::tl -> acid_of_triplet a b c::acid_decoder tl
  | _ -> []
  end

let rec acids_of_helix (x: helix) : acid list =
  begin match x with 
  | T::A::C::tl -> Met::acid_decoder tl
...
let rec add_ancestor_labels (t: tree) : labeled_tree =
  begin match t with 
  | Leaf x -> LLeaf x 
  | Node (lt, rt) -> LNode (add_ancestor_labels lt, guess_parent_helix 
  (helix_of_tree (add_ancestor_labels lt)) 
  (helix_of_tree (add_ancestor_labels rt)), add_ancestor_labels rt)
  end 
...
let rec parent_child_hamming (t: labeled_tree) : int =
  begin match t with 
  | LLeaf x -> 0
  | LNode (lt, x, rt) -> hamming_distance (helix_of_tree lt) x + 
...
let rec simplest_tree (ts: labeled_tree list) : labeled_tree * int =
  begin match ts with 
  | [] -> failwith "simplest_tree: cannot pass an empty list"
  | [x] -> (x, parent_child_hamming x)
  | (x::y::tl) -> if parent_child_hamming x <= parent_child_hamming y 
                  then simplest_tree (x::tl)
                  else simplest_tree (y::tl)
  end
